...
`uv.lock` 和 `app/InstaxApp.xcodeproj/project.pbxproj` 是工具生成文件，不要手工修改版本字段。
...
- 两处都等于目标版本：跳到验证。
- 任一处不一致或 `MARKETING_VERSION` 尚不存在：只直接编辑 `pyproject.toml` 和 `app/project.yml`，然后执行：
